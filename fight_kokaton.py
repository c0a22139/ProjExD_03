...
class Beam:
    def __init__(self, bird):
        ...
        self.rct.centery = bird.rct.centery #こうかとんの中心横座標
        self.vx, self.vy = +5, 0
...
class Bomb:
    ...
    def __init__(self):
        ...
        self.vx = random.choice(__class__.directors)
        self.vy = random.choice(__class__.directors)
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.QUIT:
                return
            if event.type == pg.KEYDOWN and event.key == pg.K_SPACE: #キーが押された、かつ、キーがスペースキーだったら
                beam = Beam(bird)
